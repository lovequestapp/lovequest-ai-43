...
export const login = async (email: string, password: string): Promise<{ success: boolean, user?: User }> => {
  try {
    ...
    const premiumStatus = profileData?.premium_status || 'basic';
    ...
    if (validPremiumStatus === 'trial' && profileData?.trial_end_date) {
...
export const register = async (email: string, password: string, name: string): Promise<{ success: boolean, user?: User }> => {
  try {
    ...
    const user: User = {
      ...
      role: 'subscriber',
      isBanned: false,
      verificationStatus: 'unverified' as 'verified' | 'unverified' | 'pending' | 'rejected',
      lastMessage: '',
      lastMessageTime: new Date(),
      status: 'offline',
      favoriteMusic: [],
...
export const getUser = async (userId: string): Promise<User | null> => {
  ...
  const premiumStatus = data.premium_status || 'basic';
  const validPremiumStatus = (premiumStatus === 'basic' || premiumStatus === 'premium' || premiumStatus === 'vip')
    ? premiumStatus as 'basic' | 'premium' | 'vip'
    : 'basic' as const;
  ...
  const verificationStatus = data.is_verified ? 'verified' : 'unverified';
  const validVerificationStatus = verificationStatus as 'verified' | 'unverified' | 'pending' | 'rejected';
